...
<body>
    <?php
            ...
            //2. Lấy dữ liệu
            $id_tham_gia = $_POST["txtID"];
            $id_su_kien = $_POST["txtSukien"];

            //3. Viết câu lệnh truy vấn sửa dữ liệu vào bảng tin tức trong CSDL
                $sql = "
                    UPDATE `tbl_tham_gia` 
                    SET `id_su_kien` = '".$id_su_kien."'
                    WHERE `tbl_tham_gia`.`id_tham_gia` = '".$id_tham_gia."'
                    ";
            ...
            //5. Hiện thị thông báo thêm mới thành công và đẩy các bạn về trang quản trị tin tức
                echo "
                    <script type='text/javascript'>
                        window.alert('Bạn đã cập nhật tham gia thành công');
                    </script>
                ";
                echo "
                    <script type='text/javascript'>
